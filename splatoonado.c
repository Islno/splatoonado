    ...
    // Ciclo de cores (cores quase apagadas, exceto o branco)
    uint8_t color_cycle[][3] = {
        {50, 50, 50}, // Branco (intensidade máxima)
        {1, 0, 0},       // Vermelho (quase apagado)
        {0, 0, 1},       // Azul (quase apagado)
        {0, 1, 0},       // Verde (quase apagado)
        {1, 1, 0},       // Amarelo (quase apagado)
        {1, 0, 1},       // Magenta (quase apagado)
        {0, 1, 1},       // Ciano (quase apagado)
        {1, 0, 1},       // Roxo (quase apagado)
        {1, 1, 0}        // Laranja (quase apagado)
    };
        ...
        while (true) {
            ...
            if (to_ms_since_boot(get_absolute_time()) - last_move_time > MOVE_DELAY) {
                ...
                if (new_led != current_led) {
                    ...
                    if (previous_led != -1) {
                        color_index[previous_led] = (color_index[previous_led] + 1) % 9;  // 9 cores no ciclo
            ...
            // Atualiza LED atual para branco (intensidade máxima)
            npSetLED(current_led, 255, 255, 255);
            npWrite();
